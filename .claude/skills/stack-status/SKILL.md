...
2. Web:
   ...
   - Chequea `http://localhost:4200` con `curl` → Vite responde si está listo.
...
if [ "$SHOW_WEB" = "1" ]; then
  ...
  code="$(curl -s -o /dev/null -w '%{http_code}' --max-time 3 http://localhost:4200 || echo 000)"
  case "$code" in
    200) echo "  HTTP: ✓ 200 — http://localhost:4200" ;;
...
- El puerto de Vite asumido es `4200` (default). Si lo cambiás en `pld-web/vite.config.ts`, hay que ajustar esta skill y `stack-logs`.
