...
(Previously: `rfc`, `curp` y `fechaNacimiento` eran requeridos en FE y BE.)
...
(Previously: `numeroTelefono` aparecía como requerido en documentación.)
...
(Previously: el domicilio (calle, número exterior, CP, divisiones) era requerido.)
...
(Previously: `POST /admin/registration` requería `rfc` en el body.)
