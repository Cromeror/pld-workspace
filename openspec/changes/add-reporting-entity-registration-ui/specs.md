# Specs: Reporting Entity Registration UI

Spec nueva para el flujo end-to-end del wizard. Crear bajo `openspec/specs/reporting-entity-registration-ui/spec.md` durante el archive.

## Behavioral invariants (Given/When/Then)
...
**Then** el sistema SHALL abrir un modal "Ya existe un registro en progreso. ¿Reanudar?"
...
**Given** el draft retornado tiene `status = IN_PROGRESS` y no expirado,
**When** se valida el draft,
**Then** el sistema SHALL mostrar modal "¿Continuar registro existente o empezar uno nuevo?"
**AND** si "Continuar": pre-llenar el state desde el draft y `goTo` al step según `current_step`,
**AND** si "Empezar nuevo": ejecutar `DELETE /:id` (Q21), limpiar localStorage, reset state.
...
### INV-13 — Cambio de profileType destruye el draft

**Given** el usuario está en step 1 con `registrationId !== undefined` (draft activo de un retomar),
**When** cambia profileType o userRole y clickea para confirmar el cambio,
**Then** el sistema SHALL mostrar ConfirmDialog "Cambiar el tipo descartará el borrador actual..."
**AND** si confirma: `DELETE /:id`, limpiar localStorage, mantener la nueva selección,
**AND** si cancela: revertir la selección al original.

### INV-14 — Country dropdown visualmente activo no interactivo
...
(Documentado como deuda técnica en design.md hasta que el diseño Figma agregue el input `cellphone` o el BE flexibilice la validación.)
