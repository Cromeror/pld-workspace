# Tech Review: auxiliar-registro-workspace-admin
...
**Backend (pld-api):**
1. Migración TypeORM: columna `username` varchar(60) unique nullable en `UserEntity`.
2. `CreateAuxiliaryDto`: agregar `username` como campo requerido (3-60 chars, alfanumérico + guiones/puntos). `email` se vuelve opcional.
3. `AuxiliariesService.create()`: crear `UserEntity` con `username`. Persistir `workspace.id` del JWT en `AuxiliaryProfileEntity`. Eliminar envío de email post-commit.
4. `LocalStrategy`/`AuthService.validateUser()`: si el identifier corresponde a un AUXILIARY, hacer lookup por `username`; si no, por `email`.

**Frontend (pld-web):**
1. `UserDataStep.tsx` + `schemas.ts`: agregar campo `username` ('Nombre de usuario') como requerido. `email` pasa a opcional.
2. `types/auxiliary.ts` + `auxiliariesService.ts`: actualizar payload con `username` requerido, `email` opcional.
3. Step 3 de confirmación: mostrar `username` como identificador principal.
