...
`JwtStrategy.validate()` en `apps/auth-users/src/shared/auth/jwt.strategy.ts` MUST comparar el claim `iat` del JWT contra `users.password_changed_at` del usuario referenciado por `sub`. Si `iat * 1000 < user.password_changed_at`, MUST rechazar con HTTP 401.

La response 401 MUST seguir el shape de error uniforme definido en la spec `http-error-formatting` y MUST incluir un código de error identificable como `password-changed` para que el frontend pueda diferenciar este caso de un 401 genérico.
...
- THEN el servidor SHALL responder HTTP 401
- AND el body SHALL seguir el shape de error uniforme
- AND el body SHALL contener un código identificable como `password-changed`
...
- GIVEN un usuario con `users.password_changed_at = NULL`
...
- WHEN un cliente envía un request a un endpoint protegido
- THEN el servidor SHALL responder HTTP 401
- AND el body NO SHALL contener el código `password-changed`
